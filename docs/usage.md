...
In this section, we will look at various exampls of using `RepoAuditor`. For the sake of brevity, we will omit the `--includes` and `--GitHub-url` flags, leaving the inclusion to the user.
...
`RepoAuditor` has built-in support for this as well, by using the `--excludes` flag.

To exclude a requirement, you can run specify the requirement as `Module-Requirement`

```sh
uvx repoauditor --includes GitHub --excludes GitHub-RequireSignedCommitsRule
```
...
```sh
uvx repoauditor --includes GitHub --excludes CommunityStandards
```

NOTE: This way of excluding a module is redundant since any module not added via `--includes` is automatically excluded.
